...
export async function loader(args: Route.LoaderArgs) {
	const returnTo = safeReturnTo(
		new URL(args.request.url).searchParams.get('returnTo'),
	);
	const auth = await getAuth(args);
	if (auth.isAuthenticated) {
		throw redirect(returnTo, { headers: authResponseHeaders });
	}
	return data({ returnTo }, { headers: authResponseHeaders });
}
...
function SignInForm({ returnTo }: { returnTo: string }) {
	...
	const inlineError =
		formError ?? formatClerkError(fieldError) ?? formatClerkError(globalError);
			...
			<CardContent>
				...
				<p className='mt-4 text-center text-sm text-muted-foreground'>
					...
					<Link
						...
						to={{
							pathname: '/sign-up',
							search: `?${new URLSearchParams({ returnTo })}`,
						}}
					>
						Sign up
					</Link>
...
export default function SignIn({ loaderData }: Route.ComponentProps) {
	return (
		<main className='mx-auto flex min-h-screen max-w-5xl items-center justify-center px-4 py-10'>
			<SignInForm returnTo={loaderData.returnTo} />
		</main>
	);
}
